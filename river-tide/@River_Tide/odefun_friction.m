...
function f = odefun_friction(obj,f,k,Q,QQ,Qhr,h0,w0,cD,c,D1_dx)
	...
	% f1 : Q''
	% f2 : Q'
	f(:,2) = f(:,2) + fl.*s.*(   -3./(1i*omega*w).*c(:,1).*Qhr  ...
				   + -3./(1i.*omega.*w).*c(:,2).*Q0 );
	...
	DQ3k1 = fourier_mulitplicative_interaction_coefficients(DQ2k1,Q,size(Q,2),1);
	...
	f(:,4) = f(:,4) + s.*( - c(:,3).*QQ(:,k+1) ...
			       + fl.*( -3./(1i.*omega.*w).*c(:,2).*DQ2k1(:,k+1) ...
                                       -3./(1i.*omega.*w).*c(:,3).*DQ3k1(:,k+1));
end % odefun_friction
