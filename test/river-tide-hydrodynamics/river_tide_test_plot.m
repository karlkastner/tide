...
function river_tide_test_plot(tid,rt,z,name,pflag)
	if (pflag)
		...
		clf();
		subplot(2,4,1);
		plot(rt.channel(1).x,[rt.zb(),rt.z(0)]);
		legend('z_b','z_0');
		...
		subplot(2,4,2);
		plot(rt.channel(1).x,abs(rt.z(1)));
		hold on;
		plot(x,abs(z),'--');
		legend('|z_1|');
		ylim([0,max(ylim())]);
		...
		subplot(2,4,6)
		plot(rt.channel(1).x,angle(rt.z(1)));
		hold on;
		plot(x,angle(z),'--');
		legend('arg(z_1)');
		ylim(pi*[-1,1]);
...
%		u2 = rt.velocity(2);
		...
		subplot(2,4,4)
		plot(rt.channel(1).x,abs(rt.Q(1)));
		legend('|Q_1|');
		ylim([0,max(ylim())]);

		subplot(2,4,8)
		plot(rt.channel(1).x,angle(rt.Q(1)));
		legend('arg(Q_1)');
		ylim(pi*[-1,1]);
